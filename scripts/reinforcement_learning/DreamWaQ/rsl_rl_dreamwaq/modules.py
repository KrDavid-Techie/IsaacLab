...
class ActorCritic_DWAQ(nn.Module):
    def __init__(self, num_actor_obs, num_critic_obs, num_actions, cenet_in_dim, cenet_out_dim, activation="elu", init_noise_std=1.0, **kwargs):
        super().__init__()

        self.is_recurrent = False
        self.activation = get_activation(activation)
        actor_input_dim = num_actor_obs
        self.obs_dim = num_actor_obs - cenet_out_dim
    ...
    def act(self, observations, obs_history, **kwargs):
        ...
        self.update_distribution(torch.cat((observations,code.detach()),dim=-1))
    ...
    def act_inference(self, obs_history, observations=None):
        # Handle TensorDict or dict input
        if hasattr(obs_history, "keys") and "policy" in obs_history.keys():
            obs_history = obs_history["policy"]

        if observations is None:
            observations = obs_history[:, :self.obs_dim]
        code,_,_,_,_,_,_ = self.cenet_forward(obs_history)
        actions_mean = self.actor(torch.cat((observations,code),dim=-1))
        return actions_mean
