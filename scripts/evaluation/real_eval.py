...
class CsvLogLoader:
    ...
    def __init__(self, file_path_pattern):
        # file_path_pattern example: logs/real_log_2026-01-06_13-39-47 (suffix like _cat.csv is automatically matched)
        self.run_prefix = file_path_pattern.replace(".csv", "").split("_base")[0] # Attempt to remove suffixes like base, imu, etc.
        
        # If the user specified '..._base.csv', improve prefix extraction logic
    ...
    def load(self):
        """
        Find files (base, imu, joint, bms, foot) and load data into a unified dictionary.
        """
        # Check file existence (based on base file)
        base_file = f"{self.run_prefix}_base.csv"
        if not os.path.exists(base_file):
            # Check legacy (single file) log compatibility
...
class RealPerformanceEvaluator:
    ...
    def evaluate(self):
        ...
        # ---------------------------------------------------------
        # Priority 1: High Level Velocity (Sensor/SportMode)
        ...
        if mean_speed > 0.01:
            ...
            # Priority 2: Logged Estimated Velocity (if "est_vx" column exists)
            ...
            # Priority 3: Post-Calculated Estimate (Offline Computation)
            else:
                velocity_source = "Computed Estimate (Offline)"
                estimator = RobustVelocityEstimator()
                # Check if self.real dictionary has been converted to numpy arrays
                # It has already been converted in the Loader, so pass as is
                est_vx = estimator.compute_velocity(self.real)
                final_speed_data = np.abs(est_vx) # Forward speed magnitude
...
def main():
    ...
    # 1. Target file directly specified
    if args.file:
        target_path = args.file
    
    # 2. Automatically search for the latest log in the directory
    else:
